...
Verifies FR-03 (sensor fusion) and NFR-PER-01 (RTK accuracy)
"""
import sys
...
class TestEKFPrediction:
    def test_predict_increases_uncertainty(self):
        ...
        imu_data = {'ax': 0.0, 'ay': 0.0, 'az': 9.81, 'gx': 0.0, 'gy': 0.0, 'gz': 0.0}
    ...
    def test_predict_updates_position_with_motion(self):
        ...
        imu_data = {'ax': 0.5, 'ay': 0.0, 'az': 9.81, 'gx': 0.0, 'gy': 0.0, 'gz': 0.0}
        for _ in range(10):
            ekf.predict(imu_data, dt=0.1)
...
class TestEKFGNSSUpdate:
    def test_gnss_update_reduces_uncertainty(self):
        ...
        gnss_data = {'is_fixed': True, 'x': 100.0, 'y': 50.0, 'z': 120.0}
    ...
    def test_gnss_unfixed_does_not_update(self):
        ...
        assert abs(state_after['y'] - 999.0) > 50.0
